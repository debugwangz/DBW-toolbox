...
def get_fan_sino_param(image, param):
    angles = np.linspace(param['startangle'], param['endangle'],param['n_proj'], endpoint=False)
    sino = get_fan_sino(image, source_ori=param['dso'], ori_detector=param['dod'], dect_w=param['detector_width'],
...
def recon_fan_param(sino, param):
    angles = np.linspace(param['startangle'], param['endangle'],param['n_proj'], endpoint=False)
    image = recon_fan(alg=param['algorithm'], sino=sino, source_ori=param['dso'], ori_detector=param['dod'],
